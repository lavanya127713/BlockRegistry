...
class _transferOwnershipState extends State<transferOwnership> {
  ...
  Future<void> uploadDocument(bytes) async {
    ...
    try {
      cid = await uploadDocument2("docname.jpeg", context, filebytes: bytes);
  ...
  Future<bool> transferOwnershipFunction() async {
    try {
      if (connectedWithMetamask) {
        await model2.transferOwnership(widget.reqId, cid);
      } else {
        await model.transferOwnership(BigInt.parse(widget.reqId), cid);
      }
